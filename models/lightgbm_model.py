...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: LGBMClassifier) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: LGBMClassifier, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "LightGBM") -> None:
...
if __name__ == "__main__":
    ...
    lgb_model = LGBMClassifier(**MODEL_PARAMS)

    start_index = int(len(df) * TRAIN_SPLIT_RATIO)
    results = run_backtest(df, lgb_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "LightGBM")
