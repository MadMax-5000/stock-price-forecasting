"""
Gradient Boosting Stock Price Prediction Model.

Gradient boosting classifier for predicting stock price direction using
...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: GradientBoostingClassifier) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: GradientBoostingClassifier, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "Gradient Boosting") -> None:
...
if __name__ == "__main__":
    ...
    results = run_backtest(df, gb_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "Gradient Boosting")
