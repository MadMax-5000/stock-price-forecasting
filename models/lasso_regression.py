"""
Lasso Regression Stock Price Prediction Model.

Lasso regression classifier for predicting stock price direction using
...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: Lasso) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: Lasso, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "Lasso Regression") -> None:
...
if __name__ == "__main__":
    ...
    lasso_model = Lasso(**MODEL_PARAMS)

    start_index = int(len(df) * TRAIN_SPLIT_RATIO)
    results = run_backtest(df, lasso_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "Lasso Regression")
