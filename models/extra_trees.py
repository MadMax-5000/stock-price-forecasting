"""
Extra Trees Stock Price Prediction Model.

Extra trees classifier for predicting stock price direction using
...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: ExtraTreesClassifier) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: ExtraTreesClassifier, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "Extra Trees") -> None:
...
if __name__ == "__main__":
    ...
    et_model = ExtraTreesClassifier(**MODEL_PARAMS)

    start_index = int(len(df) * TRAIN_SPLIT_RATIO)
    results = run_backtest(df, et_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "Extra Trees")
