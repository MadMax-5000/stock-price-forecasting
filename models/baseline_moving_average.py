...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Generate moving average feature.

    Args:
        df: Input DataFrame with Close column.

    Returns:
        DataFrame with MA feature added.
    """
    result_df = df.copy()
    result_df[f"MA_{MA_WINDOW}"] = result_df["Close"].rolling(window=MA_WINDOW).mean()
    return result_df.dropna()


def predict_signal(test: pd.DataFrame) -> pd.DataFrame:
    """
    Return predictions based on price vs moving average.

    Args:
        test: Test DataFrame with Close and Target columns.
...
def train_and_evaluate(
    ...
    """
    Run Moving Average baseline backtest.
    ...
    print_benchmark_table(results, "Moving Avg", show_extended=show_extended)
