"""
Ridge Regression Stock Price Prediction Model.

Ridge regression classifier for predicting stock price direction using
...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: RidgeClassifier) -> pd.DataFrame:
    """Trains model and returns predictions based on optimized threshold."""
    model.fit(train[predictors], train["Target"])
    
    probs = model.decision_function(test[predictors])
...
def run_backtest(data: pd.DataFrame, model: RidgeClassifier, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "Ridge Regression") -> None:
...
if __name__ == "__main__":
    ...
    ridge_model = RidgeClassifier(**MODEL_PARAMS)

    start_index = int(len(df) * TRAIN_SPLIT_RATIO)
    results = run_backtest(df, ridge_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "Ridge Regression")
