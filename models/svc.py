"""
Support Vector Classification Stock Price Prediction Model.

SVC classifier for predicting stock price direction using
...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: SVC, scaler: StandardScaler) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: SVC, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "SVC") -> None:
...
if __name__ == "__main__":
    ...
    results = run_backtest(df, svc_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "SVC")
