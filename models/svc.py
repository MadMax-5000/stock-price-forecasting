...
def create_model(**kwargs: Any) -> SVC:
    """
    Create an SVC model with given or default parameters.
...
def run_backtest_svc(
    ...
    """
    Execute walk-forward validation for SVC with scaling.
...
def train_and_evaluate(
...
) -> tuple[pd.DataFrame, SVC]:
    """
    Train SVC model and run backtest.
    ...
    results = run_backtest_svc(
        df, model, features, start=start_index, step=step, threshold=threshold
    )

    print_benchmark_table(results, "SVC", show_extended=show_extended)
