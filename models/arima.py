...
ARIMA time series model for predicting stock price direction using
...
MODEL_PARAMS: dict[str, tuple[int, int, int] | str] = {
    "order": (5, 1, 0),
    "trend": "c",
}
...
def predict_signal(train: pd.Series, test: pd.Series) -> pd.DataFrame:
    ...
    Args:
        train: Training series (Close prices).
        test: Test series (Target values).
    ...
    """
    try:
        model = ARIMA(train, **MODEL_PARAMS)
        fitted = model.fit()
...
def train_and_evaluate(
    ...
    """
    Train ARIMA model and run backtest.
    ...
    print_benchmark_table(results, "ARIMA", show_extended=show_extended)
