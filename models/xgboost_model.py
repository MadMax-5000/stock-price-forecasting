...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: XGBClassifier) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: XGBClassifier, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "XGBoost") -> None:
...
if __name__ == "__main__":
    ...
    results = run_backtest(df, xgb_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "XGBoost")
