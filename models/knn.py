...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: KNeighborsClassifier, scaler: StandardScaler) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: KNeighborsClassifier, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "KNN") -> None:
...
if __name__ == "__main__":
    ...
    results = run_backtest(df, knn_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "KNN")
