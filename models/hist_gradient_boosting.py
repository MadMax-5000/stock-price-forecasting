...
def predict_signal(train: pd.DataFrame, test: pd.DataFrame, predictors: list[str], model: HistGradientBoostingClassifier) -> pd.DataFrame:
...
def run_backtest(data: pd.DataFrame, model: HistGradientBoostingClassifier, predictors: list[str], start: int, step: int) -> pd.DataFrame:
...
def print_benchmark_table(predictions: pd.DataFrame, model_name: str = "Hist Gradient Boosting") -> None:
...
if __name__ == "__main__":
    ...
    hgb_model = HistGradientBoostingClassifier(**MODEL_PARAMS)

    start_index = int(len(df) * TRAIN_SPLIT_RATIO)
    results = run_backtest(df, hgb_model, features, start=start_index, step=BACKTEST_STEP)

    print_benchmark_table(results, "Hist Gradient Boosting")
